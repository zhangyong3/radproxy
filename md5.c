...
/* forward declaration */
static void Transform (unsigned int *, unsigned int *);
...
int md5_calc(void *digest, const void *msg, int len)
{
	MD5_CTX ctx;
	int i;

	MD5Init (&ctx);
	MD5Update (&ctx, (const unsigned char*)msg, len);
	MD5Final (digest, &ctx);

}


void lrad_hmac_md5(const void *text, int text_len,
	      const void *key, int key_len,  void *digest)
{
	MD5_CTX context;
